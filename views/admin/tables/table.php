...
$statuses = $CI->jobreports_model->get_statuses();
        ...
        elseif ($aColumns[$i] == 'status') {
            ...
            $span = '';
                //if (!$locked) {
                    ...
                    foreach ($statuses as $jobreportChangeStatus) {
                        if ($aRow['status'] != $jobreportChangeStatus) {
                            $span .= '<li>
                          <a href="#" onclick="jobreport_mark_as(' . $jobreportChangeStatus . ',' . $aRow['id'] . '); return false;">
                             ' . format_jobreport_status($jobreportChangeStatus) . '
                          </a>
                       </li>';
                        }
                    }
            ...
            if ($aRow['status'] == 1) {
                $outputStatus = '<span class="label label-danger inline-block">' . _l('jobreport_status_draft') . $span;
            } elseif ($aRow['status'] == 2) {
                $outputStatus = '<span class="label label-info inline-block">' . _l('jobreport_status_sent') . $span;
            } elseif ($aRow['status'] == 3) {
                $outputStatus = '<span class="label label-default inline-block">' . _l('jobreport_status_declined') . $span;
            } elseif ($aRow['status'] == 4) {
                $outputStatus = '<span class="label label-success inline-block">' . _l('jobreport_status_accepted') . '</span>';
            } elseif ($aRow['status'] == 5) {
                $outputStatus = '<span class="label label-primary inline-block">' . _l('jobreport_status_expired') . $span;
            }
